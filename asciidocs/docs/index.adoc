...
^.^|*Unternehmen*
^.^|image:redhat-logo.png[width=80, height=80]
^.^|image:okta-logo.png[width=80, height=80]
^.^|404
^.^|image:authentik-logo.png[width=80, height=80]
...
=== Lösung
*Keycloak* bietet eine link:https://www.keycloak.org/docs-api/latest/rest-api/index.html[REST API^] an, um Konfigurationen vorzunehmen. Die link:https://quarkus.io/guides/security-keycloak-admin-client[quarkus-keycloak-admin-rest-client^] dependency bietet Methoden, um  jene REST API in einer *Quarkus-Applikation* verwenden zu können.
...
=== Quarkus-Applikation mit Keycloak authentifizieren
image:keycloak-quarkus-connection.png[]
Damit die Quarkus-App im Keycloak Änderungen vornehmen kann, muss sie mit diesem authentifiziert werden. Dafür müssen ein Realm ([red]*rot*) und ein Client ([green]*grün*) in diesem Realm erstellt werden. Dies wird in den folgenden 2 Abschnitten erklärt.
