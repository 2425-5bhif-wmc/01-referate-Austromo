...
Der "Create flow" button tut, was er sagt und erstellt einen Leeren Flow dies wird von Keycloak nicht empfohlen, da man eher bestehende Flows Duplizieren soll. Wenn ein Flow dann Editieren oder Ansehen will, kann auf den Namen gedrückt werden, um diese Ansicht zu öffnen:
...
Um jetzt unseren Authentifikator zu entwickeln, müssen die "Service Provider Interfaces" von Keycloak implementiert werde, da wir auch credentials benötigen müssen wir auch dafür die entsprechenden Interfaces implementieren.
...
* Es alles innerhalb eines einzigen jar file sein, weswegen wir jat.type=uber-jar verwenden
[code, toml]
----
...
IMPORTANT: Es muss sichergestellt werden das, wenn ein neuer Subflow als Required gesetzt wird muss dieser auch unter dem Required actions tab auf on gesetzt werden.
